...
public class Life extends PApplet {
    ...
    public int countCellsAround(int row, int col) {
        ...
        /*
         * if (getCell(board, row - 1, col - 1)) { count++; } if (getCell(board, row -
         * 1, col)) { count++; } if (getCell(board, row - 1, col + 1)) { count++; } if
         * (getCell(board, row, col - 1)) { count++; } if (getCell(board, row, col + 1))
         * { count++; } if (getCell(board, row + 1, col - 1)) { count++; } if
         * (getCell(board, row - 1, col)) { count++; } if (getCell(board, row + 1, col +
         * 1)) { count++; }
         */
    ...
    public void randomize() {
        for (int row = 0; row < size; row++) {
            for (int col = 0; col < size; col++) {
                ...
                /*
                 * if (dice < 0.5) { board[row][col] = true; }
                 * 
